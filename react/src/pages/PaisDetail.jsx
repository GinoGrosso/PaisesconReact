...
function PaisDetail() {
  ...
  if (error) return <ErrorMessage message="Error al cargar el país" />;
        ...
        <div id="detalle-container">
              ...
              <ul className="info-list">
                {idiomas.map((idioma) => (
                  <li key={idioma.codigo}>
                    <strong>{idioma.codigo.toUpperCase()}:</strong> {idioma.nombre}
                  </li>
                ))}
              </ul>
            </div>
          ...
          {monedas.length > 0 && (
            <div className="detalle-section">
              ...
              <ul className="info-list">
                {monedas.map((moneda) => (
                  <li key={moneda.codigo}>
                    <strong>{moneda.codigo}:</strong> {moneda.nombre} {moneda.simbolo ? `(${moneda.simbolo})` : ""}
                  </li>
                ))}
              </ul>
            </div>
